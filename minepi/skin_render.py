...
def matrix_mult(A, B):
    """ Multiplies two 3x3 matrices represented as lists of lists. """
...
class Point:
    def __init__(self, super_cls, origin_coords: list):
        self.is_projected: bool = False
        self.super = super_cls
        self.origin_coords: list = origin_coords
        self.dest_coords: Optional[list] = None

    @property
    def depth(self):
        return self.dest_coords[2]

    def project(self, offset: list, rotation_matrix: list):
        def matmul(vec, mat):
            return [sum(vec[i] * mat[i][j] for i in range(len(vec))) for j in range(len(mat[0]))]

        translated = [self.origin_coords[i] - offset[i] for i in range(len(offset))]
        rotated = matmul(translated, rotation_matrix)
        final_coords = [rotated[i] + offset[i] for i in range(len(offset))]
        self.dest_coords = matmul(final_coords, self.super.body_angles["general"])
...
class Render:
    ...
    def calculate_rotation_matrix(cls, ry, rx):
        return matrix_mult(cls.rotation_y(ry), cls.rotation_x(rx))
    ...
    def calculate_angles(self):
        ...
        r = matrix_mult(self.rotation_y(beta), self.rotation_x(alpha))
        self.body_angles["general"] = r

        # ToDo: think of a better way to do this
        # currently: apply part specific rotation -> apply offset -> apply general rotation
        self.body_angles["torso"] = matrix_mult(self.rotation_y(0), self.rotation_x(0))
    ...
    def determine_faces(self):
        ...
        for k, v in self.visible_faces.items():
            ...
            for cube_point in self.cube_points:
                cube_point[0].project([0, 0, 0], self.body_angles[k])  # torso is always level with the plane
    ...
    def generate_polygons(self, hd_ratio, skin, im_cape):
        ...
        for i in range(0, 9 * hd_ratio):
            for j in range(0, 9 * hd_ratio):
                volume_points = append_dict(volume_points, i, j, -2 * hd_ratio,
                                            Point(self, [i, j, -2 * hd_ratio]))
                volume_points = append_dict(volume_points, i, j, 6 * hd_ratio,
                                            Point(self, [i, j, 6 * hd_ratio]))

        for j in range(0, 9 * hd_ratio):
            for k in range(-2 * hd_ratio, 7 * hd_ratio):
                volume_points = append_dict(volume_points, 0, j, k, Point(self, [0, j, k]))
                volume_points = append_dict(volume_points, 8 * hd_ratio, j, k,
                                            Point(self, [8 * hd_ratio, j, k]))

        for i in range(0, 9 * hd_ratio):
            for k in range(-2 * hd_ratio, 7 * hd_ratio):
                volume_points = append_dict(volume_points, i, 0, k, Point(self, [i, 0, k]))
                volume_points = append_dict(volume_points, i, 8 * hd_ratio, k,
                                            Point(self, [i, 8 * hd_ratio, k]))
        ...
        if self.display_hair:
            ...
            for i in range(0, 9 * hd_ratio):
                for j in range(0, 9 * hd_ratio):
                    volume_points = append_dict(volume_points, i, j, -2 * hd_ratio,
                                                Point(self, [i * 8.5 / 8 - 0.25 * hd_ratio,
            ...
            for j in range(0, 9 * hd_ratio):
                for k in range(-2 * hd_ratio, 7 * hd_ratio):
                    volume_points = append_dict(volume_points, 0, j, k,
                                                Point(self, [-0.25 * hd_ratio,
                                                                        j * 8.5 / 8 - 0.25 * hd_ratio,
                                                                        k * 8.5 / 8 - 0.25 * hd_ratio]))
                    volume_points = append_dict(volume_points, 8 * hd_ratio, j, k,
                                                Point(self, [8.25 * hd_ratio,
                                                                        j * 8.5 / 8 - 0.25 * hd_ratio,
                                                                        k * 8.5 / 8 - 0.25 * hd_ratio]))
        ...
        if not self.head_only:
            ...
            for i in range(0, 9 * hd_ratio):
                for j in range(0, 13 * hd_ratio):
                    volume_points = append_dict(volume_points, i, j, 0,
                                                Point(self, [i, j + 8 * hd_ratio, 0]))
                    volume_points = append_dict(volume_points, i, j, 4 * hd_ratio,
                                                Point(self, [i, j + 8 * hd_ratio, 4 * hd_ratio]))

            for j in range(0, 13 * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    volume_points = append_dict(volume_points, 0, j, k,
                                                Point(self, [0, j + 8 * hd_ratio, k]))
                    volume_points = append_dict(volume_points, 8 * hd_ratio, j, k,
                                                Point(self, [8 * hd_ratio, j + 8 * hd_ratio, k]))
            ...
            if self.layers:
                ...
                for i in range(0, 9 * hd_ratio):
                    for j in range(0, 13 * hd_ratio):
                        volume_points = append_dict(volume_points, i, j, 0,
                                                    Point(self, [i * 8.25 / 8 - 0.125 * hd_ratio,
                ...
                for j in range(0, 13 * hd_ratio):
                    for k in range(0, 5 * hd_ratio):
                        volume_points = append_dict(volume_points, 0, j, k,
                                                    Point(self, [-0.125 * hd_ratio,
                                                                          (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio,
                                                                          k * 4.25 / 4 - 0.125 * hd_ratio]))
                        volume_points = append_dict(volume_points, 8 * hd_ratio, j, k,
                                                    Point(self, [8.125 * hd_ratio,
                                                                          (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio,
                                                                          k * 4.25 / 4 - 0.125 * hd_ratio]))
            ...
            if self.display_cape:
                ...
                for i in range(0, 11 * hd_ratio):
                    for j in range(0, 17 * hd_ratio):
                        volume_points = append_dict(volume_points, i, j, 0,
                                                    Point(self, [i - 1, j + 8 * hd_ratio, -1]))
                        volume_points = append_dict(volume_points, i, j, 1 * hd_ratio,
                                                    Point(self, [i - 1, j + 8 * hd_ratio, 0]))
            ...
            for i in range(start, 5 * hd_ratio):
                for j in range(0, 13 * hd_ratio):
                    volume_points = append_dict(volume_points, i, j, 0,
                                                Point(self, [i - 4 * hd_ratio, j + 8 * hd_ratio, 0]))
                    volume_points = append_dict(volume_points, i, j, 4 * hd_ratio,
                                                Point(self, [i - 4 * hd_ratio, j + 8 * hd_ratio, 4 * hd_ratio]))

            for j in range(0, 13 * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    volume_points = append_dict(volume_points, start, j, k,
                                                Point(self, [-4 * hd_ratio + start, j + 8 * hd_ratio, k]))
            ...
            for i in range(start, 5 * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    volume_points = append_dict(volume_points, i, 0, k,
                                                Point(self, [i - 4 * hd_ratio, 8 * hd_ratio, k]))
                    volume_points = append_dict(volume_points, i, 12 * hd_ratio, k,
                                                Point(self, [i - 4 * hd_ratio, 20 * hd_ratio, k]))
            ...
            if self.layers:
                ...
                for i in range(start, 5 * hd_ratio):
                    for j in range(0, 13 * hd_ratio):
                        volume_points = append_dict(volume_points, i, j, 0,
                                                    Point(self, [(i * 4.25 / 4 - 0.125 * hd_ratio) - 4 * hd_ratio,
                                                                          (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio,
                                                                          -0.125 * hd_ratio]))
                        volume_points = append_dict(volume_points, i, j, 4 * hd_ratio,
                                                    Point(self, [(i * 4.25 / 4 - 0.125 * hd_ratio) - 4 * hd_ratio,
                ...
                for j in range(0, 13 * hd_ratio):
                    for k in range(0, 5 * hd_ratio):
                        volume_points = append_dict(volume_points, start, j, k,
                                                    Point(self, [(-4.125 + start) * hd_ratio,
                                                                          (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio,
                                                                          k * 4.25 / 4 - 0.125 * hd_ratio]))
                        volume_points = append_dict(volume_points, 4 * hd_ratio, j, k,
                                                    Point(self, [0.125 * hd_ratio,
            ...
            for i in range(0, (5 - start) * hd_ratio):
                for j in range(0, 13 * hd_ratio):
                    volume_points = append_dict(volume_points, i, j, 0,
                                                Point(self, [i + 8 * hd_ratio, j + 8 * hd_ratio, 0]))
                    volume_points = append_dict(volume_points, i, j, 4 * hd_ratio,
                                                Point(self, [i + 8 * hd_ratio, j + 8 * hd_ratio, 4 * hd_ratio]))
            ...
            for j in range(0, 13 * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    ...
                    volume_points = append_dict(volume_points, (4 - start) * hd_ratio, j, k,
                                                Point(self, [(12 - start) * hd_ratio, j + 8 * hd_ratio, k]))

            for i in range(0, (5 - start) * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    volume_points = append_dict(volume_points, i, 0, k,
                                                Point(self, [i + 8 * hd_ratio, 8 * hd_ratio, k]))
                    volume_points = append_dict(volume_points, i, 12 * hd_ratio, k,
                                                Point(self, [i + 8 * hd_ratio, 20 * hd_ratio, k]))
            ...
            if self.layers:
                ...
                for i in range(0, (5 - start) * hd_ratio):
                    for j in range(0, 13 * hd_ratio):
                        volume_points = append_dict(volume_points, i, j, 0,
                                                    Point(self, [(i * 4.25 / 4 - 0.125 * hd_ratio) + 8 * hd_ratio,
                                                                          (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio,
                                                                          -0.125 * hd_ratio]))
                        volume_points = append_dict(volume_points, i, j, 4 * hd_ratio,
                                                    Point(self, [(i * 4.25 / 4 - 0.125 * hd_ratio) + 8 * hd_ratio,
                ...
                for j in range(0, 13 * hd_ratio):
                    for k in range(0, 5 * hd_ratio):
                        volume_points = append_dict(volume_points, 0, j, k,
                                                    Point(self, [7.875 * hd_ratio,
                                                                          (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio,
                                                                          k * 4.25 / 4 - 0.125 * hd_ratio]))
                        volume_points = append_dict(volume_points, (4 - start) * hd_ratio, j, k,
                                                    Point(self, [(12.125 - start) * hd_ratio,
                                                                          (j * 12.25 / 12 - 0.125 * hd_ratio) + 8 * hd_ratio,
                                                                          k * 4.25 / 4 - 0.125 * hd_ratio]))
            ...
            for i in range(0, 5 * hd_ratio):
                for j in range(0, 13 * hd_ratio):
                    volume_points = append_dict(volume_points, i, j, 0,
                                                Point(self, [i, j + 20 * hd_ratio, 0]))
                    volume_points = append_dict(volume_points, i, j, 4 * hd_ratio,
                                                Point(self, [i, j + 20 * hd_ratio, 4 * hd_ratio]))

            for j in range(0, 13 * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    volume_points = append_dict(volume_points, 0, j, k,
                                                Point(self, [0, j + 20 * hd_ratio, k]))
                    volume_points = append_dict(volume_points, 4 * hd_ratio, j, k,
                                                Point(self, [4 * hd_ratio, j + 20 * hd_ratio, k]))
            ...
            if self.layers:
                ...
                for i in range(0, 5 * hd_ratio):
                    for j in range(0, 13 * hd_ratio):
                        volume_points = append_dict(volume_points, i, j, 0,
                                                    Point(self, [i * 4.25 / 4 - 0.125 * hd_ratio,
            ...
            for i in range(0, 9 * hd_ratio):
                for j in range(0, 13 * hd_ratio):
                    volume_points = append_dict(volume_points, i, j, 0,
                                                Point(self, [i + 4 * hd_ratio, j + 20 * hd_ratio, 0]))
                    volume_points = append_dict(volume_points, i, j, 4 * hd_ratio,
                                                Point(self, [i + 4 * hd_ratio, j + 20 * hd_ratio, 4 * hd_ratio]))
            ...
            for j in range(0, 13 * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    volume_points = append_dict(volume_points, 0, j, k,
                                                Point(self, [4 * hd_ratio, j + 20 * hd_ratio, k]))
                    volume_points = append_dict(volume_points, 4 * hd_ratio, j, k,
                                                Point(self, [8 * hd_ratio, j + 20 * hd_ratio, k]))
            ...
            for i in range(0, 9 * hd_ratio):
                for k in range(0, 5 * hd_ratio):
                    volume_points = append_dict(volume_points, i, 0, k,
                                                Point(self, [i + 4 * hd_ratio, 20 * hd_ratio, k]))
                    volume_points = append_dict(volume_points, i, 12 * hd_ratio, k,
                                                Point(self, [i + 4 * hd_ratio, 32 * hd_ratio, k]))
            ...
            if self.layers:
                ...
                for i in range(0, 5 * hd_ratio):
                    for j in range(0, 13 * hd_ratio):
                        ...
                        volume_points = append_dict(volume_points, i, j, 4 * hd_ratio,
                                                    Point(self,
                                                          [(i * 4.25 / 4 - 0.125 * hd_ratio) + 4 * hd_ratio,
                                                                    (j * 12.25 / 12 - 0.125 * hd_ratio) + 20 * hd_ratio,
                                                                    4.125 * hd_ratio]))
    ...
    def member_rotation(self, hd_ratio):
        for face in self.polygons["head"]:
            for poly in self.polygons["head"][face]:
                poly.project(
                    [4 * hd_ratio, 8 * hd_ratio, 2 * hd_ratio],
        ...
        if not self.head_only:
            ...
            for face in self.polygons["r_arm"]:
                for poly in self.polygons["r_arm"][face]:
                    poly.project(
                        [-2 * hd_ratio, 10 * hd_ratio, 2 * hd_ratio],
            ...
            for face in self.polygons["l_arm"]:
                for poly in self.polygons["l_arm"][face]:
                    poly.project(
                        [10 * hd_ratio, 10 * hd_ratio, 2 * hd_ratio],
            ...
            for face in self.polygons["r_leg"]:
                for poly in self.polygons["r_leg"][face]:
                    poly.project(
                        [2 * hd_ratio, 22 * hd_ratio, 2 * hd_ratio],
            ...
            for face in self.polygons["l_leg"]:
                for poly in self.polygons["l_leg"][face]:
                    poly.project(
                        [6 * hd_ratio, 22 * hd_ratio, 2 * hd_ratio],
            ...
            for face in self.polygons[body_part]:
                for poly in self.polygons[body_part][face]:
                    if not poly.is_projected:
                        poly.project([0, 0, 0], self.body_angles["torso"])
...
class Polygon:
    ...
    def project(self, offset, rotation_matrix):
        for dot in self.dots:
